...
export function addProvidersToDatabase (db, providerInput) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < enteredProviders.length; i++) {
      ...
      if (specialtyList[specialtyName] === undefined) {
        specialtyList[specialtyName] = null
      }
    }
    ...
    addSpecialtiesToDatabase(db, specialtyList).then((specialtyDict) => {
      ...
      sqlProvidersToDatabase(db, providerList).then((result) => { resolve(result) })
    })
  })
}
...
function sqlProvidersToDatabase (db, providerList) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < providerList.length; i++) {
      ...
      const providerVal = `("${provider.firstName}", "${provider.middleName}", "${provider.lastName}", ${specialtyName}, ${provider.specialtyId}), `
      values += providerVal
    }
